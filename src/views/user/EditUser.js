...
import { adminAllActionUser } from 'api/Endpoints'
...
function EditUser() {
    ...
    uploadBytes(storageRef, selectedImage)
      .then(snapshot => {
        getDownloadURL(ref(storage, `${imageName}`)).then(url => {
          ...
          const url1 = adminAllActionUser(state?.id)
  ...
  useEffect(() => {
    if (state?.id) {
      const url = adminAllActionUser(state?.id)
      axiosInstance
        .get(url)
        .then(
          res => (
            console.log('res', res.data.data),
            setSelectedImage(res.data.data.user.profileImage),
            console.log(
              'profileImage',
